...
- 安全组放行 **3020**（若用 Nginx 则放行 **80/443**，后端只监听本机 3020 即可）。
...
若浏览器访问的**页面域名**与 **API 域名**不一致，增加例如：`CORS_ORIGINS=https://你的前端域名`；同源（只开 :3020 或 Nginx 同一域名反代）通常**不用**配。
...
.venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 3020 --workers 1
```

浏览器访问：`http://<公网IP>:3020`（存在 `frontend/dist` 时由后端一并提供静态页）。

---

## 6.（可选）Nginx 反代 HTTPS

参考同目录 **`nginx-promanager.conf.example`**，修改 `server_name`、证书路径，并把 `proxy_pass` 指向 `127.0.0.1:3020`。
