...
```text
python -m uvicorn app.main:app --reload --host 127.0.0.1 --port 3020
```
...
```text
.venv\Scripts\python.exe -m uvicorn app.main:app --reload --host 127.0.0.1 --port 3020
```

Linux / macOS 未激活 venv 时：

```text
.venv/bin/python -m uvicorn app.main:app --reload --host 127.0.0.1 --port 3020
```
...
开发时一般 **两个终端各跑上面一条**：API 与文档在 <http://127.0.0.1:3020>（含 `/docs`），页面在 <http://127.0.0.1:3008>（Vite 会把 `/api` 转到 3020）。
...
- **`connect ECONNREFUSED 127.0.0.1:3020`**：3020 上 **没有进程在听**，即 **后端没启动或已退出**。请先起 `uvicorn`。
- **`read ECONNRESET`**：连上后被对端掐断，多见于 **热重载重启** 或进程崩溃。

表示浏览器经 Vite 访问 `/api` 时，连到 `127.0.0.1:3020` 出问题。常见原因：
...
3. **想更稳**：调前端时可先 **去掉 `--reload`** 跑后端；或只监视 `app` 并加大重载间隔，例如：  
   `python -m uvicorn app.main:app --reload --reload-dir app --reload-delay 1 --host 127.0.0.1 --port 3020`
...
cd ../backend && python -m uvicorn app.main:app --reload --host 127.0.0.1 --port 3020
```

然后浏览器打开 <http://127.0.0.1:3020>（存在 `frontend/dist` 时后端会顺带提供静态页）。
...
开发模式下 Vite 把 `/api` 代理到 `http://127.0.0.1:3020`，所以要先起后端再起前端。

## 定时自动生成报告

可用系统计划任务在每天固定时间调用：

- `GET http://127.0.0.1:3020/api/reports/daily.md?date=...`  
- `GET http://127.0.0.1:3020/api/reports/weekly.md?week_start=...`  
